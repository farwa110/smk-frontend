    ...
    >
      <motion.div animate={{ rotateY: flipped ? 180 : 0 }} transition={{ duration: 1.5, delay: 0.2, ease: "easeInOut" }} className="relative w-full h-full" style={{ transformStyle: "preserve-3d" }}>
        ...
        <div className="absolute w-full h-full backface-hidden">
          {/* Artwork Image */}
          <Image src={data.image} alt={data.title} fill className="object-cover" sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, 33vw" />

          {/* SVG Frame Overlay*/}

          <Image src={lightgray} alt="Museum Frame" className="absolute inset-0 w-full h-full pointer-events-none z-10 scale-[1.05]" sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, 33vw" />
        </div>
